...
class OrderController extends Controller
{
    ...
    function SendToDelivery(Request $request)
    {
        ...
        if ($order->status != 'pending' && $order->status != 'not_paid') {
            return redirect()->route('orders.index')->with('error', 'Order is not in a valid state for delivery.');
        }
        ...
        $email = new OrderShipped($order, $otp);
    ...
    public function markAsPaid(Order $order)
    {
        ...
        if ($order->status !== 'not_paid' && $order->delivery_id == null) {
            return redirect()->route('orders.index')->with('error', 'Invalid order status for marking as paid.');
        }


        // Add a payment record and associate it with the order
        $order->payment()->create([
            'user_id' => $order->user->id,
            'order_id' => $order->id,
            'payment_method' => 'payment_link',
        ...
        return redirect()->route('orders.index')->with('success', 'Order marked as paid successfully.');
    }
}
